...
resource "aws_lb_listener" "internal_rke2_agent_port_9345" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.external_rke2_agent_9345_tg.arn
  }  
}

resource "aws_lb_target_group" "external_rke2_agent_9345_tg" {
  name     = "external-nlb-rke2-9345-TG"
  port     = 9345
  protocol = "TCP"
  vpc_id   = aws_vpc.test_vpc.id
}
...
resource "aws_lb_listener" "external_rke2_server_port_80" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.external_rke2_server_80_tg.arn
  }
}
...
resource "aws_lb_listener" "external_rke2_server_port_443" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.external_rke2_server_443_tg.arn
  }
}
...
resource "aws_lb_target_group" "external_rke2_server_80_tg" {
  name     = "external-nlb-rke2-TG-80"
  port     = 80
  protocol = "TCP"
  vpc_id   = aws_vpc.test_vpc.id
}

resource "aws_lb_target_group" "external_rke2_server_443_tg" {
  name     = "external-nlb-rke2-TG-443"
  port     = 443
  protocol = "TCP"
  vpc_id   = aws_vpc.test_vpc.id
}
